...
处理规则（仅适用于抖音-竞价和抖音-品牌）：
1. 合并目录下所有Excel文件
2. 提取并按广告主账号id求和：总消耗、预付消耗+预借款消耗、授信消耗、赠款消耗、返佣消耗
3. 若返佣消耗有数据，预付消耗+预借款消耗 = 预付消耗+预借款消耗 - 返佣消耗
4. 回填到导入模板（日期为昨天）

模板列映射（回填数据导入模板_头条.xlsx）：
  *日期 → 昨天日期
  *投放账户ID → 广告主账户id
  *总消耗 → 总消耗
  现金消耗 → 预付消耗+预借款消耗（扣除返佣后）
  信用消耗 → 授信消耗
  赠款消耗 → 赠款消耗
  返佣消耗 → 返佣消耗

用法：
  python 抖音千川消耗处理.py                    # 处理今天的日期目录
  python 抖音千川消耗处理.py 2026-07-07         # 处理指定日期目录

目录结构：
  原始文件: {日期}/原始文件/抖音-竞价- 消耗/  (放入侠客行+新美两个Excel)
           {日期}/原始文件/抖音-品牌- 消耗/  (放入侠客行+新美的Excel)
  生成文件: {日期}/生成文件/抖音-竞价- 消耗/
           {日期}/生成文件/抖音-品牌- 消耗/
...
# 模板文件路径
TEMPLATE_PATH = SCRIPT_DIR / "回填数据导入模板_头条.xlsx"
...
# 原始数据列名
RAW_COLUMNS = [
    '时间', '广告主账户id', '广告主账户名称', '广告主公司id',
    '广告主公司名称', '广告类型', '总消耗', '预付消耗+预借款消耗',
    '授信消耗', '赠款消耗', '返佣消耗'
]
...
def read_all_xlsx(raw_dir: Path) -> pd.DataFrame:
    ...
    if not all_data:
        raise RuntimeError(f"目录中未读取到有效数据: {raw_dir}")

    return pd.concat(all_data, ignore_index=True)


def process_douyin(df: pd.DataFrame, platform_name: str, output_dir: Path, date_str: str) -> dict:
    """处理抖音/千川消耗数据"""
    ...
    # 检查必需列
    missing_cols = [col for col in SUM_COLUMNS + ['广告主账户id'] if col not in df.columns]
    if missing_cols:
    ...
    print(f"规则1 - 按广告主账户id求和: {len(grouped)} 个账户")
    ...
    if TEMPLATE_PATH.is_file():
        ...
        template_header = pd.read_excel(TEMPLATE_PATH, sheet_name='头条', dtype=str, nrows=0)
...
def main():
    ...
    for platform_name in PROCESS_PLATFORMS:
        ...
        try:
            ...
            result = process_douyin(df, platform_name, output_dir, date_str)
            results.append((platform_name, result, None))
        except Exception as e:
            print(f"❌ {platform_name} 处理失败: {e}")
            results.append((platform_name, None, str(e)))

        print()

    # 提示不需要处理的平台
    print("=" * 60)
    print("📋 以下平台不需要处理（直接上传DSP）:")
    for platform_name in UPLOAD_ONLY_PLATFORMS:
        raw_dir = date_dir / "原始文件" / platform_name
        if raw_dir.is_dir():
            files = [f for f in raw_dir.glob("*.xlsx") if not f.name.startswith('~$')]
            if files:
                print(f"  📁 {platform_name}: {len(files)} 个文件")
            else:
                print(f"  📁 {platform_name}: 目录为空")
    ...
    if failures:
        ...
        log_path = log_dir / f"抖音千川消耗处理日志_{timestamp}.txt"
        lines = [f"抖音千川消耗处理日志 - {date_str}", f"生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", "=" * 60, ""]
